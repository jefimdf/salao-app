...
export default function AgendaView(props) {
  ...
  const dataString = (data) => {
    ...
    let dia = (((new Date(data))).getDate()).toString().length === 1 ? '0' + (new Date(data)).getDate() : (new Date(data)).getDate();
    let mes = ((new Date(data)).getMonth() + 1).toString().length === 1 ? '0' + ((new Date(data)).getMonth() + 1) : ((new Date(data)).getMonth() + 1);

    return (new Date(data)).getFullYear() + '' + mes + '' + dia
  };
